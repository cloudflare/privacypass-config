...
async function startService(serviceName: string, app: AppConfig) {
  ...
  switch (serviceName) {
    case 'attester': {
      const childProcess = execa('npm', ['run', 'dev', '--', '--port', deployments.attester.port, '--var', `ISSUER_DIRECTORY_URL:"${deployments.issuer.url}${PRIVATE_TOKEN_ISSUER_DIRECTORY}`, '--var', `ISSUER_REQUEST_URL:"${deployments.issuer.url}/token-request"`], { cwd, detached: true })
      childProcess.pipeStdout!(process.stdout)
      break;
    }
    case 'issuer': {
      execa('npm', ['run', 'dev', '--', '--port', deployments.issuer.port], { cwd, detached: true }).pipeStdout!(process.stdout)
      await new Promise(resolve => setTimeout(resolve, 5000))
      fetch(`${deployments.issuer.url}/admin/rotate`, { method: 'POST' })
      break;
    }
    case 'origin': {
      execa('npm', ['run', 'dev', '--', '--port', deployments.origin.port, '--var', `ISSUER_NAME:"${new URL(deployments.issuer.url).host}`, '--var', `ORIGIN_NAME:"${new URL(deployments.origin.url).host}"`], { cwd, detached: true }).pipeStdout!(process.stdout)
      break
    }
  }
